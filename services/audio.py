...
def string_to_audio(encuded_audio):
    raw_bytes = base64.b64decode(encuded_audio)
...
def generate_clip(text, voice):
    ...
    if response.status_code == 200:
        ...
        audio = AudioSegment.from_file(audio_bytes, format="mp3") + AudioSegment.silent(duration=500)
...
def generate_audio(quotes, av_db):
    ...
    print("length of clips:", len(clips))
...
def build_tomes(clips):
    ...
    for group in groupings:
        # duration = 0 causes issues with converting to string and back. RIP the time it took to figure that one out.
        sum_audio = AudioSegment.silent(duration=1)
        text = ""
        sentences = []
        for clip in group:
